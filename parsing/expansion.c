...
/*   Updated: 2024/02/07 09:30:27 by ricardo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (node->comando[i][j] != '\0')
		{
			...
			if (node->comando[i][j] == '$')
			{
				...
				if (node->comando[i][j] == '?')
				{
					...
					node->comando[i] = change_exit(node->comando[i],
						string_exit, 2);
					free(string_exit);
					continue ;
				}
				start = j;
				while (node->comando[i][j] != '\0' && node->comando[i][j] != ' '
					&& node->comando[i][j] != '\t'
					&& validate_chars_env(node->comando[i][j]) == 0)
				{
					j++;
				}
				...
				if (tmp_l_env == NULL)
				{
					node->comando[i] = change_exit(node->comando[i], "", (j
							- start + 1));
				}
				free(env_input);
			}
